...
- **Only one instance may run**: `open` on a live instance only hands the launch to the running
  ...
  unique executable name is what makes `pkill -f TachiSmoke` safe; check with `pgrep -fl Tachi`
- **Run the suite SERIALLY — one invocation at a time.** The drivers are timing-sensitive in a way that
  ...
  was a 0.4s-interval front-app sampler (one `osascript` spawn per tick), while the same tree passed 18/18 in
  1m23s with the sampler left at 3s. Before believing a failure, check `pgrep -fl "itest/desktop|TachiSmoke"`
  and re-run the scenario on its own: the synthetic DRAGS are the flakiest of all (a second gesture is
  sometimes swallowed), so a lone drag assertion failing inside a full run is the harness until proven
  otherwise.
...
- **A covered window is no longer a paused run — demo mode keeps the page awake, deliberately.**
  ...
  transient answers). The same heartbeat then reads 304ms / 60fps / `visible` while covered. **The run no longer needs the foreground window**: the whole
  suite was verified with its window covered for the entire run (18/18 in 1m23s), so `make desktop-smoke`
