...
无法解析的命令保守判为 ask（在非交互场景等于拒绝）。判据是**语法真的不完整**（引号未闭合、here-doc 未闭合），不是"命令很长/多行"：命令按**原文**解析，多行与 heredoc 都正常处理 —— 早先版本会先把空白压成单空格再解析（那是为已删除的"精确命令串会话记忆"做的），于是任何 heredoc 的定界符都不再位于行首、正文里的引号变成活语法，导致 `cat > f <<'EOF'` 这种最常用的写法一律落到保守 ask。正文是**数据**：它不出现在段里，不会被规则命中。

会话级开关（`AllowAllAsksForSession`）**不覆盖**这个兜底：它在任何规则被查看之前就返回了，所以解析不了的命令每次都要确认。语义上这是有意的——开关的安全性来自"deny 在 ask 之前判定"，而无法解析时根本没有 deny 判定可依赖。
