...
- **可观测的基线**：修复后、且工具集不变时，重启的第一次请求应当几乎全部命中（实测 2026-09-13 13:19 的一次重启：未命中 342 token = 本轮自己的 reminder + 新消息，命中 484,736 → 99.9%）。反之，重启后首请求出现整段未命中，就说明前缀里又有东西漂了 —— 按 `api_requests.jsonl` 的断点层级定位（system+tools 之后就断 = 工具集或首条消息变了；只断在中后段 = 新跑过的回合与重建形式不一致）。注意**工具集变化是结构性的**：前缀以 tools 开头，任何工具增删（MCP 连接、skill/plan 开关、升级带来的新工具）都会让后面整段历史按未命中重算。
