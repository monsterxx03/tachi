...
> 历史行快照不受影响。
>
> **9/10 更新**：官方 2026-09-10 12:00（北京时间）起调整 **flash 系列**
> （含 deepseek-v4-flash-vision-exp）定价：空闲时段 缓存命中 ¥0.02 / 缓存未命中
> 输入 ¥1 / 输出 ¥4，高峰时段 = 空闲 ×2（¥0.04 / ¥2 / ¥8），时段表不变
> （周一至周五峰谷、周末全天空闲）。deepseek-v4-pro 不在本次调整内。
> 生效时刻为 12:00 而非 00:00 —— 9/10 当天 09:00-12:00 的高峰仍按 8/24 版价。
> 内置表新增第四版本（`EffectiveFrom: 2026-09-10 12:00`）。
