...
- **每个项目都占一个组头，含还没有会话的项目**：组头是它的 ＋ 与右键菜单唯一的入口，空项目因此显示一行"还没有会话"。
- **`＋ 新建会话` 属于「无项目」组**：它造出的会话不属于任何项目，所以它长在无项目组头之上，而不是挂在侧栏顶部（顶部是全宽的着色按钮，比会话本身还抢眼，读起来像整页的主操作）。它是这一组自己的 ＋。
- **⌘N 在眼前的会话所属的项目里新建**：当前会话由某个项目驱动时，新会话绑定同一个项目（工作区、附加根、技能都来自项目，掉到项目外等于静默换掉一整套环境）；否则新建无项目会话。判据与分组一致：悬空 `project_id` 的会话界面上就在「无项目」组里（§5、§8.6），⌘N 也就按无项目处理。
