...
func (t *TachiAgent) NewSession(ctx context.Context, req acp.NewSessionRequest) (acp.NewSessionResponse, error) {
	...
	aiAgent, mcpMgr, err := agent.NewAIAgentWithConfig(configureCtx, agent.AgentConfig{
		...
		ACPFileMode:     t.clientCapabilities != nil && t.clientCapabilities.Fs.WriteTextFile,
		ACPReadMode:     t.clientCapabilities != nil && t.clientCapabilities.Fs.ReadTextFile,
		ACPTerminalBash: t.clientCapabilities != nil && t.clientCapabilities.Terminal,
...
func (t *TachiAgent) UnstableDeleteSession(ctx context.Context, req acp.UnstableDeleteSessionRequest) (acp.UnstableDeleteSessionResponse, error) {
	...
	// Delete from disk.
	sm, err := session.NewManager(t.logger)
	if err != nil {
...
func (t *TachiAgent) ListSessions(ctx context.Context, req acp.ListSessionsRequest) (acp.ListSessionsResponse, error) {
	...
	diskMgr, err := session.NewManager(t.logger)
	if err == nil {
...
func (t *TachiAgent) ResumeSession(ctx context.Context, req acp.ResumeSessionRequest) (acp.ResumeSessionResponse, error) {
	...
	// prefers the session's stored provider (when it still resolves), else
	// falls back to the main provider.
	providerResolved, err := providerForLoadedSession(t.cfg, loaded.ProviderName)
	if err != nil {
...
func (t *TachiAgent) LoadSession(ctx context.Context, req acp.LoadSessionRequest) (acp.LoadSessionResponse, error) {
	...
	// Resolve provider: the session's stored provider when available
	// (matches ResumeSession behavior), else the main provider.
	loadedName := ""
	if loaded != nil {
		loadedName = loaded.ProviderName
	}
	providerResolved, err := providerForLoadedSession(t.cfg, loadedName)
	if err != nil {
	...
	if sm != nil {
		if loaded != nil {
			...
			if _, err := sm.New(providerResolved.Provider.ProviderName(), cwd); err != nil {
				t.logger.Warn(ctx, fmt.Sprintf("ACP: failed to create session: %v", err))
			}
			aiAgent.SetSessionManager(sm)
		}
...
// the session's stored provider name when it still resolves, else the main
// provider (stale names — provider removed from config — fall through
// silently). The provider carries its config name (BuildProvider /
