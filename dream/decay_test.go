...
func TestCalculateDecay(t *testing.T) {
	// Zero time → brand new, decay = 1.0
	if d := CalculateDecay(time.Time{}); d != 1.0 {
		t.Errorf("zero time: expected 1.0, got %f", d)
	}

	// Just now → close to 1.0
	now := time.Now()
	d := CalculateDecay(now)
	if d < 0.99 || d > 1.01 {
