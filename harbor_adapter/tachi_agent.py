...
Usage:
    ...
    # Run Terminal-Bench with Tachi (defaults to deepseek-v4-pro):
...
class TachiAgent(BaseInstalledAgent):
    ...
    async def install(self, environment: BaseEnvironment) -> None:
        ...
        # 4. Generate config.yaml with plaintext API keys from host environment
    ...
    def _generate_config_yaml(self) -> str:
        """Generate a config.yaml with plaintext API keys from host env vars."""
        model = self.model_name or "deepseek-v4-pro"
        model_clean = model
        ...
        # Determine provider type, API key, and base URL from host env
        provider_type = "openai"
        api_key = os.environ.get("OPENAI_API_KEY", "")
        base_url = ""

        if model_clean.startswith("deepseek") or model.startswith("deepseek/"):
        ...
        # If BASE_URL env var is set on the host, force it into config
        # (overrides any hardcoded default like DeepSeek's endpoint)
        if provider_type == "anthropic" and os.environ.get("ANTHROPIC_BASE_URL"):
            base_url = os.environ["ANTHROPIC_BASE_URL"]
        elif provider_type == "openai" and os.environ.get("OPENAI_BASE_URL"):
            base_url = os.environ["OPENAI_BASE_URL"]

        if not api_key:
            self.logger.warning(
                "No API key found in host environment for provider '%s'", provider_type
            )

        # Read max_iterations from env (used by bench.sh via TACHI_MAX_ITERATIONS)
        max_iterations = os.environ.get("TACHI_MAX_ITERATIONS", "50")

        config = f"""# Auto-generated by Tachi Harbor adapter
language: en
