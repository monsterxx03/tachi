...
// stubPriceSource is a fixed in-memory llm.PriceScheduleSource — the tests
// here exercise ResolveModelPriceAt's semantics (band inheritance, version
// pick, timezone, pinning) without config.Config; the config side (wiring
// YAML structures into llm.PricingConfig) is covered by its own tests.
type stubPriceSource struct {
	pricing *PricingConfig
}

func (s stubPriceSource) PricingSchedule(string) *PricingConfig {
	return s.pricing
}
...
func TestResolveModelPriceAt_Bands(t *testing.T) {
	src := stubPriceSource{pricing: &PricingConfig{
		InputPrice:  f64(1.0),
		OutputPrice: f64(2.0),
		Bands: []PriceBandSpec{
	...
	in := ResolveModelPriceAt(src, "custom", "some-model", atHour(10))
	...
	out := ResolveModelPriceAt(src, "custom", "some-model", atHour(13))
...
func TestResolveModelPriceAt_BandInheritance(t *testing.T) {
	...
	rp := ResolveModelPriceAt(src, "custom", "m", atHour(2)) // inside 23:00-07:00
...
func TestResolveModelPriceAt_BandParseFailureSkipped(t *testing.T) {
	...
	rp := ResolveModelPriceAt(src, "custom", "m", atHour(10))
...
func TestResolveModelPriceAt_BandsOnlyUsesBuiltinFlat(t *testing.T) {
	...
	rp := ResolveModelPriceAt(src, "ds", "deepseek-chat", atHour(10))
...
func TestResolveModelPriceAt_Timezone(t *testing.T) {
	...
	rp := ResolveModelPriceAt(src, "custom", "m", time.Date(2026, 8, 17, 6, 30, 0, 0, time.UTC))
	...
	rp = ResolveModelPriceAt(src, "custom", "m", time.Date(2026, 8, 17, 1, 30, 0, 0, time.UTC))
...
func TestResolveModelPriceAt_UnknownModelBandsOnly(t *testing.T) {
	...
	rp := ResolveModelPriceAt(src, "custom", "no-such-model", atHour(10))
	...
	out := ResolveModelPriceAt(src, "custom", "no-such-model", atHour(13))
...
func TestResolveModelPriceAt_BandsReplaceBuiltin(t *testing.T) {
	...
	rp := ResolveModelPriceAt(src, "ds", "deepseek-chat", postPeak)
...
// TestResolveModelPriceAt_NoSource: nil source → built-in table alone
