...
func TestBuildSessionMessagesCarriesChange(t *testing.T) {
	...
	msgs := buildSessionMessages(raw, 0, nil)
	if len(msgs) != 3 {
...
func TestReviewChangesGuards(t *testing.T) {
	...
	if got := svc.ReviewChanges("", 0, nil, ""); !strings.Contains(got, "没有可评审的改动") {
		t.Errorf("empty scope = %q, want a refusal", got)
	}
	if got := svc.ReviewChanges("", 0, []string{"a.go"}, ""); !strings.Contains(got, "活跃会话") {
